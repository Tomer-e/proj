...
def k_test(filename,k,download_time, to_log_file=False):
    ...
    for j in range(k):
        ...
        for var in last_chunk_bit_rate[j]:
            l = utils.VIDEO_BIT_RATE[-1]/utils.VIDEO_BIT_RATE[-1] # - MARABOU_ERR # highest definition // 4300/4300
            u = utils.VIDEO_BIT_RATE[-1]/utils.VIDEO_BIT_RATE[-1] # + MARABOU_ERR # highest definition // 4300/4300
            network.setLowerBound(var, l)
            network.setUpperBound(var, u)
            if j == 0:
        ...
        for var in next_chunk_sizes[j]:
            ...
            if size_i == 0:
                ...
                eq = MarabouUtils.Equation(EquationType=MarabouCore.Equation.EQ)
                eq.addAddend(-1, var)
                eq.addAddend(utils.VIDEO_BIT_RATE[size_i]/utils.VIDEO_BIT_RATE[0], next_chunk_sizes[j][0])
                eq.setScalar(0)
                network.addEquation(eq)
            size_i += 1
    ...
    # Call to C++ Marabou solver
        ...
        # output_file = sys.stdout if not OUTPUT_TO_FILE else 1 # TODO!!!
